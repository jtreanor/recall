...
- **Overlay shows a brief left-edge gap on open** — *re-characterized 2026-06-11; this was originally filed as "opening/closing animation has a slight horizontal drift" and chased as an animation bug for a month. There were two distinct artifacts hiding under that one report:*
  ...
  **Approaches to explore next** (ordered by promise / cost):
  ...
  - **M. Never order the panel out — keep the backdrop warm.** Direct consequence of the 2026-06-11 re-characterization: the settle happens because every `show()` does `makeKeyAndOrderFront` on a window the Window Server has never composited (every `hide()` calls `orderOut`). Instead: order the panel in **once** (at launch or first show) and keep it in the window list permanently at `alphaValue = 0` when "hidden"; `show()` sets `alphaValue = 1` and runs the original smooth `animator().setFrame` slide; `hide()` slides out then drops alpha to 0 instead of ordering out. The backdrop never cold-starts, so the first-composite settle should never run on open. Public API only, preserves the original feel exactly. Verify with the instant-show test first (alpha-flip with no slide: settle gone?), then the full slide. Watch for: WindowServer cost of a permanently-composited invisible blur window; `canJoinAllSpaces` behavior while invisible; whether alpha 0 still participates in screen captures/Mission Control.
